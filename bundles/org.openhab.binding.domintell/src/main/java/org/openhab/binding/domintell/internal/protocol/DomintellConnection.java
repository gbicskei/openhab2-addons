...
import org.openhab.binding.domintell.internal.protocol.StateListener.State;
...
public class DomintellConnection {
     ...
     */
    private StateListener.State currentState = State.OFFLINE;
    ...
    /**
     * Turned to true while receiving APPINFO content
     */
    private boolean appInfoCycle = false;
     ...
     */
    public DomintellConnection(BridgeConfig config, StateListener stateListener,
            ConfigurationEventHandler configEventListener, ScheduledExecutorService scheduler) {
        this.config = config;
    ...
    public boolean isAppInfoCycle() {
        return appInfoCycle;
    }

    private void updateState(StateListener.State state, @Nullable String msg) {
        ...
        if (oldState != StateListener.State.ONLINE && state == StateListener.State.ONLINE) {
            registry.getModules().forEach(m -> sendCommand(
                    ActionMessageBuilder.createFromModuleKey(m.getModuleKey()).withAction(ActionType.STATUS).build()));
        }
        if (state != oldState) {
            if (msg != null) {
    ...
    public void startGateway(BridgeConfig config) {
        ...
        if (config.isValid()) {
            ...
            readerCheckJob = scheduler.scheduleWithFixedDelay(this::checkReader, 0, READER_CHECK_DELAY,
                    TimeUnit.SECONDS);
        } else {
    ...
    public @Nullable Date getDomintellSysdate() {
    ...
    private void startBackgroundThreads() {
        try {
            ...
            senderThread = new MessageSenderThread((@NonNull DatagramSocket) socket);
            senderThread.start();

            readerThread = new MessageReceiverThread((@NonNull DatagramSocket) socket);
            readerThread.start();

            // send HELLO command after every 50s according to the Domintell protocol requirements to keep the
            // connection open
    ...
    private void stopBackgroundThreads() {
        ...
        if (socket != null) {
            @NonNull
            DatagramSocket nnSocket = (@NonNull DatagramSocket) socket;
            if (nnSocket.isConnected()) {
                nnSocket.disconnect();
            }
            if (!nnSocket.isClosed()) {
                nnSocket.close();
            }
            socket = null;
        }
        ...
        public void run() {
            while (true) {
                try {
                    ...
                    try {
                        DatagramPacket p = new DatagramPacket(buf, buf.length, config.getInternetAddress(),
                                config.getPort());
                        socket.send(p);
                    } catch (IOException e) {
    ...
    private class MessageReceiverThread extends Thread {
        ...
        private boolean handleMessageArray(String[] msgArray) {
            for (String msg : msgArray) {
                ...
                @Nullable
                BaseMessage baseMessage = parser.parseMessage(msg, appInfoCycle);
                if (baseMessage != null) {
         ...
         */
        private boolean handleMessage(BaseMessage message) {
            try {
        ...
        private void processModuleStatusMessage(StatusMessage message) {
            try {
                ...
                if (moduleType.isModuleSupported()) {

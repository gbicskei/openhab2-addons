...
public abstract class ContactModule extends Module {
    ...
    public void shortPush(int idx) {
        getConnection().sendCommand(ActionMessageBuilder.createFromModuleKey(getModuleKey()).withIONumber(idx)
                .withAction(ActionType.SHORT_PUSH).build());
    }
    ...
    public void longPush(int idx) {
        getConnection().sendCommand(ActionMessageBuilder.createFromModuleKey(getModuleKey()).withIONumber(idx)
                .withAction(ActionType.LONG_PUSH).build());
    }

    /**
     * Simulate double push on contact
     *
     * @param idx Contact index
     */
    public void doublePush(int idx) {
        getConnection().sendCommand(ActionMessageBuilder.createFromModuleKey(getModuleKey()).withIONumber(idx)
                .withAction(ActionType.DOUBLE_PUSH).build());
    }
}

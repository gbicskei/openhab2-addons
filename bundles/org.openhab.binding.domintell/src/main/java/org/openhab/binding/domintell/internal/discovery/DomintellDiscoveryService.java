...
@Component(service = { DiscoveryService.class,
        DomintellDiscoveryService.class }, configurationPid = "discovery.domintell")
public class DomintellDiscoveryService extends AbstractDiscoveryService {
    ...
    private void createBridgeResult() {
        ...
        DiscoveryResult discoveryResult = DiscoveryResultBuilder.create(uid).withThingType(BRIDGE_THING_TYPE)
                .withLabel("Domintell Bridge").build();
    ...
    private void createGroupResult(ItemGroup<?> group) {
        ...
        DiscoveryResult discoveryResult = DiscoveryResultBuilder.create(groupUID).withBridge(bridgeUID)
                .withLabel(group.getType().getName()).build();
        thingDiscovered(discoveryResult);
    ...
    private void createModuleResult(Module module) {
        ...
        ThingTypeUID thingTypeUID = getThigTypeUID(moduleKey.getModuleType());
        ThingUID moduleUID = new ThingUID(thingTypeUID, (@NonNull ThingUID) bridgeUID,
                moduleKey.getSerialNumber().getAddressHex());
        DiscoveryResult discoveryResult = DiscoveryResultBuilder.create(moduleUID).withBridge(bridgeUID)
                .withLabel("Domintell " + moduleKey.toLabel())
                .withProperty(CONFIG_SERIAL_NUMBER, module.getModuleKey().getSerialNumber().getAddressInt())
                .withProperty(CONFIG_MODULE_TYPE, moduleKey.getModuleType().toString()).build();
        thingDiscovered(discoveryResult);
        logger.debug("Module discovered: {}", module.getModuleKey());
    }

    private ThingTypeUID getThigTypeUID(ModuleType moduleType) {
        ThingTypeUID thingTypeUID = MODULE_THING_TYPES.get(moduleType);
        return thingTypeUID != null ? thingTypeUID : THING_TYPE_MODULE_BIR;
    }
    ...
    private ThingUID getGroupTypeUID(ItemGroup<?> group) {
        switch (group.getType()) {
            case system:
                return new ThingUID(THING_TYPE_GROUP, (@NonNull ThingUID) bridgeUID, ItemGroupType.system.toString());
            case variable:
                return new ThingUID(THING_TYPE_GROUP, (@NonNull ThingUID) bridgeUID, ItemGroupType.variable.toString());
        }
    ...
    /**
     * Create discovery result from registered groups and modules and also fires APPINFO command to scan the Domintell
